...
uint32_t m_nLevel;
uint32_t QR_m_nVersion;
uint32_t m_nMaskingNo;

uint32_t m_ncDataCodeWordBit,m_ncAllCodeWord, nEncodeVersion;

uint32_t m_ncDataBlock;

//BYTE m_byRSWork[MAX_CODEBLOCK]; //RS code word calculation work

uint32_t m_nSymbolSize;

static const QR_VERSIONINFO QR_VersionInfo[] = {{0}, // (Ver.0)
										 { 1, // Ver.1
										    26,   19,   16,   13,    9,
										   0,   0,   0,   0,   0,   0,   0,
										   1,  26,  19,
...
static const BYTE byExpTouint32_t[] = {  1,   2,   4,   8,  16,  32,  64, 128,  29,  58, 116, 232, 205, 135,  19,  38,
...
static const BYTE byuint32_tToExp[] = {  0,   0,   1,  25,   2,  50,  26, 198,   3, 223,  51, 238,  27, 104, 199,  75,
...
static const LPBYTE const byRSExp []  = {NULL,      NULL,      NULL,      NULL,      NULL,      NULL,      NULL,      byRSExp7,  NULL,      NULL,
...
static const uint32_t nIndicatorLenNumeral[]  = {10, 12, 14};
static const uint32_t nIndicatorLenAlphabet[] = { 9, 11, 13};
static const uint32_t nIndicatorLen8Bit[]	   = { 8, 16, 16};
static const uint32_t nIndicatorLenKanji[]	   = { 8, 10, 12};


uint32_t IsNumeralData(unsigned char c)
{
	if (c >= '0' && c <= '9')
		return 1;
...
uint32_t IsAlphabetData(unsigned char c)
{
	if (c >= '0' && c <= '9')
		return 1;
...
uint32_t IsKanjiData(unsigned char c1, unsigned char c2)
{
...
uint32_t SetBitStream(uint32_t nIndex, WORD wData, uint32_t ncData,BYTE m_byDataCodeWord[MAX_DATACODEWORD])
{
	uint32_t i;
...
uint32_t GetBitLength(BYTE nMode, uint32_t ncData, uint32_t nVerGroup)
{
	uint32_t ncBits = 0;
...
uint32_t EncodeSourceData(LPCSTR lpsSource, uint32_t ncLength, uint32_t nVerGroup,uint32_t m_nBlockLength[MAX_DATACODEWORD],BYTE m_byBlockMode[MAX_DATACODEWORD],BYTE m_byDataCodeWord[MAX_DATACODEWORD])
{
	...
	uint32_t ncSrcBits, ncDstBits; //The bit length of the block mode if you have over the original bit length and a single alphanumeric
	uint32_t nBlock = 0;

	while (nBlock < m_ncDataBlock - 1)
	{
		uint32_t ncJoinFront, ncJoinBehind; 		//Bit length when combined with 8-bit byte block mode before and after
		uint32_t nJoinPosition = 0; 		// Block the binding of 8-bit byte mode: combined with the previous -1 = 0 = do not bind, bind behind a =
			...
			(m_byBlockMode[nBlock] == QR_MODE_ALPHABET && m_byBlockMode[nBlock + 1] == QR_MODE_NUMERAL))
		{
			...
			if (ncSrcBits > ncDstBits)
			{
				...
				{
				// Block mode uint32_tegrated uint32_to a single alphanumeric string of numbers and alphanumeric
	...
	/////////////////////////////////////////////////////////////////////////
	// Mosquito bit array
	uint32_t ncComplete = 0;	//Data pre-processing counter
...
uint32_t GetEncodeVersion(uint32_t nVersion, LPCSTR lpsSource, uint32_t ncLength,uint32_t m_nBlockLength[MAX_DATACODEWORD],BYTE m_byBlockMode[MAX_DATACODEWORD],BYTE m_byDataCodeWord[MAX_DATACODEWORD])
{
	uint32_t nVerGroup = nVersion >= 27 ? QR_VERSION_L : (nVersion >= 10 ? QR_VERSION_M : QR_VERSION_S);
	uint32_t i, j;
...
void GetRSCodeWord(LPBYTE lpbyRSWork, uint32_t ncDataCodeWord, uint32_t ncRSCodeWord)
{
	uint32_t i, j;
	...
	for (i = 0; i < ncDataCodeWord ; ++i)
	{
		if (lpbyRSWork[0] != 0)
		{
			BYTE nExpFirst = byuint32_tToExp[lpbyRSWork[0]]; //Multiplier coefficient is calculated from the first term

			for (j = 0; j < ncRSCodeWord; ++j)
			{

				//Add (% 255  ^ 255 = 1) the first term multiplier to multiplier sections
				BYTE nExpElement = (BYTE)(((uint32_t)(byRSExp[ncRSCodeWord][j] + nExpFirst)) % 255);


				//Surplus calculated by the exclusive
				lpbyRSWork[j] = (BYTE)(lpbyRSWork[j + 1] ^ byExpTouint32_t[nExpElement]);
			}
...
void SetFinderPattern(uint32_t x, uint32_t y,BYTE m_byModuleData[MAX_MODULESIZE][MAX_MODULESIZE])
{
...
void SetVersionPattern(BYTE m_byModuleData[MAX_MODULESIZE][MAX_MODULESIZE])
{
	...
	uint32_t nVerData = QR_m_nVersion << 12;
...
void SetAlignmentPattern(uint32_t x, uint32_t y,BYTE m_byModuleData[MAX_MODULESIZE][MAX_MODULESIZE])
{
	static const BYTE byPattern[] = {0x1f,  // 11111b
							   0x11,  // 10001b
							   0x15,  // 10101b
							   0x11,  // 10001b
							   0x1f}; // 11111b
...
void SetFunctionModule(BYTE m_byModuleData[MAX_MODULESIZE][MAX_MODULESIZE])
{
	...
	//Pattern alignment
	for (i = 0; i < QR_VersionInfo[QR_m_nVersion].ncAlignPouint32_t; ++i)
	{
		SetAlignmentPattern(QR_VersionInfo[QR_m_nVersion].nAlignPouint32_t[i], 6,m_byModuleData);
		SetAlignmentPattern(6, QR_VersionInfo[QR_m_nVersion].nAlignPouint32_t[i],m_byModuleData);

		for (j = 0; j < QR_VersionInfo[QR_m_nVersion].ncAlignPouint32_t; ++j)
		{
			SetAlignmentPattern(QR_VersionInfo[QR_m_nVersion].nAlignPouint32_t[i], QR_VersionInfo[QR_m_nVersion].nAlignPouint32_t[j],m_byModuleData);
		}
	}
...
{
	uint32_t x = m_nSymbolSize;
	uint32_t y = m_nSymbolSize - 1;

	uint32_t nCoef_x = 1; 	//placement orientation axis x
	uint32_t nCoef_y = 1; 	//placement orientation axis y
...
void SetMaskingPattern(uint32_t nPatternNo,BYTE m_byModuleData[MAX_MODULESIZE][MAX_MODULESIZE])
{
	uint32_t i, j;
...
void SetFormatInfoPattern(uint32_t nPatternNo,BYTE m_byModuleData[MAX_MODULESIZE][MAX_MODULESIZE])
{
	uint32_t nFormatInfo;
	uint32_t i;
	...
	uint32_t nFormatData = nFormatInfo << 10;
...
uint32_t CountPenalty(BYTE m_byModuleData[MAX_MODULESIZE][MAX_MODULESIZE])
{
	uint32_t nPenalty = 0;
	uint32_t i, j, k;
...
void FormatModule(BYTE m_byModuleData[MAX_MODULESIZE][MAX_MODULESIZE],BYTE m_byAllCodeWord[MAX_ALLCODEWORD])
{
	...
	if (m_nMaskingNo == -1)
	{
		...
		uint32_t nMinPenalty = CountPenalty(m_byModuleData);
		...
		for (i = 1; i <= 7; ++i)
		{
			...
			uint32_t nPenalty = CountPenalty(m_byModuleData);
...
void putBitToPos(uint32_t pos,uint32_t bw,unsigned char *bits)
{
        if(bw==0) return;
        uint32_t tmp;
...
uint32_t EncodeData(uint32_t nLevel, uint32_t nVersion , LPCSTR lpsSource, uint32_t sourcelen, unsigned char QR_m_data[])
{
	...
	uint32_t ncLength = sourcelen > 0 ? sourcelen : strlen(lpsSource);
	...
	uint32_t ncDataCodeWord = QR_VersionInfo[QR_m_nVersion].ncDataCodeWord[nLevel];

	uint32_t ncTerminater = min(4, (ncDataCodeWord * 8) - m_ncDataCodeWordBit);
	...
	uint32_t nDataCwIndex = 0; 	 // Position data processing code word


	// Division number data block
	uint32_t ncBlock1 = QR_VersionInfo[QR_m_nVersion].RS_BlockInfo1[nLevel].ncRSBlock;

	uint32_t ncBlock2 = QR_VersionInfo[QR_m_nVersion].RS_BlockInfo2[nLevel].ncRSBlock;

	uint32_t ncBlockSum = ncBlock1 + ncBlock2;


	uint32_t nBlockNo = 0;			  // Block number in the process


	// The number of data code words by block
	uint32_t ncDataCw1 = QR_VersionInfo[QR_m_nVersion].RS_BlockInfo1[nLevel].ncDataCodeWord;

	uint32_t ncDataCw2 = QR_VersionInfo[QR_m_nVersion].RS_BlockInfo2[nLevel].ncDataCodeWord;


	// Code word uint32_terleaving data placement
	...
	uint32_t ncRSCw1 = QR_VersionInfo[QR_m_nVersion].RS_BlockInfo1[nLevel].ncAllCodeWord - ncDataCw1;


	uint32_t ncRSCw2 = QR_VersionInfo[QR_m_nVersion].RS_BlockInfo2[nLevel].ncAllCodeWord - ncDataCw2;
